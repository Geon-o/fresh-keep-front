...
- For any git-related actions (status checking, staging, committing, or pushing), you MUST trigger and strictly follow the **`git-commit-push` skill** defined in [.agents/skills/git-commit-push/SKILL.md](file:///C:/dev/fresh-keep/.agents/skills/git-commit-push/SKILL.md).
- Ensure explicit user approval is obtained before running any write/push commands, and strictly adhere to the repository isolation rules (never commit conversation logs or summaries to the source repositories).