    ...
    <SafeAreaView style={[styles.container, { backgroundColor }]} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
        <View style={[styles.card, { backgroundColor: cardColor, borderColor }]}>
          ...
          <Text style={[styles.bodyText, { color: descColor }]}>
            {"본 서비스는 실명, 이메일, 전화번호 등 실식별 개인정보는 수집하지 않습니다. 다만 서비스 제공을 위해 아래 정보를 수집·이용합니다.\n" +
             "1. 실제 기기 정보가 아닌, 앱이 자체적으로 무작위 생성하여 저장하는 익명 토큰(임의의 UUID) 및 데이터 동기화를 위한 연동 코드 — 특정 기기나 개인을 식별할 수 있는 정보가 아니며, 앱을 재설치하면 새로 생성됩니다.\n" +
             "2. 이용자가 선택적으로 입력하는 닉네임\n" +
             "3. 이용자가 등록한 냉장고·식재료 정보(명칭, 수량, 유통기한 등)\n" +
             "4. 위치 정보: 이용자가 별도로 권한을 허용한 경우에 한해 식중독 지수 조회 목적으로만 사용되며, 외부 날씨 API에 실시간 전달될 뿐 본 서비스의 자체 서버에는 저장되지 않습니다.\n" +
             "위 정보는 서비스 제공 목적으로만 이용되며, 이용자는 설정의 '초기화' 기능을 통해 언제든 삭제를 요청할 수 있습니다."}
          </Text>
